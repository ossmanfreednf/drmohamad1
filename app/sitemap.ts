...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const paths = ["", "/about", "/general-surgeon-lebanon", "/conditions", ...Object.keys(conditions).map(slug=>`/conditions/${slug}`), "/procedures", ...Object.keys(procedures).map(slug=>`/procedures/${slug}`), "/second-opinion", "/research-publications", "/articles", ...articlePaths, "/vip-services", "/hospitals", "/contact", "/appointment"];
